...
def db_iter_videos():
    db = db_connection()
    res = db.execute(
        "SELECT path, title, duration_sec, video_url, poster_url, date FROM video ORDER BY date DESC"
    )
    for row in res:
...
def db_scan_videos(root: Path, prefix=""):
    ...
    for path in root.glob("**/*"):
        ...
        db.execute(
            "INSERT OR IGNORE INTO video (path, video_url, title, date) VALUES (?, ?, ?, ?)",
            (str(path), video_url, title, date),
        )
        cnt += 1
    db.commit()
...
def set_duration(path: Path):
    ...
    if duration:
        db = db_connection()
        db.execute(
            "UPDATE video SET duration_sec = ? WHERE path = ?", (duration, str(path))
        )
        db.commit()
...
def set_poster(path: Path):
    ...
    if generate_poster(path, poster_path):
        ...
        db = db_connection()
        db.execute(
            "UPDATE video SET poster_url = ? WHERE path = ?", (poster_url, str(path))
        )
        db.commit()
...
def db_scan_poster(root: Path, prefix=""):
    ...
    for row in res:
        ...
        if poster_path.exists():
            ...
            poster_url = "/poster/" + path.name + ".png"
            db.execute(
                "UPDATE video SET poster_url = ? WHERE path = ?",
                (poster_url, str(path)),
            )
        else:
            cnt_queued += 1
...
def db_set_audio(path: Path, audio_path: Path):
    db = db_connection()
    db.execute(
        "UPDATE video SET audio_path = ? WHERE path = ?", (str(audio_path), str(path))
    )
    db.commit()
...
def db_scan_audio(root, prefix=""):
    ...
    for row in res:
        ...
        if audio_path.exists():
            db.execute(
                "UPDATE video SET audio_path = ? WHERE path = ?",
                (str(audio_path), str(path)),
            )
            cnt_found += 1
